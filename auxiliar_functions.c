...
char* generate_filepath(char* filename) {
	...
	memcpy(filepath+strlen(filename)-JOB_F_E_LEN, EXTENSION_STR, EXTENSION_LEN+1);
	return filepath;
}
...
int check_bytes_written(int out_file, const char* buffer, ssize_t bytes_written, ssize_t bytes_to_write){
	if (bytes_written == -1) {
		...
		while (bytes_written < bytes_to_write) {	//bytes_to_write = buffer_size-1
			...
			ssize_t new_bytes_written = write(out_file, buffer + bytes_written, (size_t)(bytes_to_write-bytes_written));	//remove null terminator
